...
export default function Message({ prevMessage, message, currentUser }) {
  ...
  const currDate = moment(message?.createdAt);
  useEffect(() => {
  ...
  return (
    <>
      {!currDate?.isSame(prevDate, 'date') && (
        <div className='flex w-full justify-center'>
        <p className='text-xs font-mono font-semibold text-center px-3 py-1 rounded-lg bg-gray-300'>
