...
export default function StatusBox({ issue }) {
  ...
  return (
    <div className='status inline-flex items-center gap-2 hs-dropdown'>
      <button id='hs-dropdown-default' className='hs-dropdown-toggle ' type='button'>
        {current === 'To Do' && (
          <span className='font-semibold focus:ring-4 focus:outline-none rounded text-xs px-4 py-2 text-center inline-flex items-center uppercase text-gray-900 bg-gray-300 hover:bg-gray-300 focus:ring-gray-200'>
            {current}{' '}
            <svg
      ...
      {current === 'Done' && (
        <button
          id='hs-dropdown-default'
          className='hs-dropdown-toggle font-semibold focus:ring-4 focus:outline-none rounded text-xs px-4 py-2 text-center inline-flex items-center uppercase text-white bg-green-700 hover:bg-green-800 focus:ring-green-300'
          type='button'
        >
          {current}{' '}
          <svg
      ...
      {/* {isLoading ? <div className="animate-spin inline-block w-5 h-5 border-[3px] border-current border-t-transparent text-blue-600 rounded-full" role="status" aria-label="loading">
          ...
          {statusArray
            ...
            .map((status) => (
              <li key={status} onClick={() => setCurrent(status)}>
                ...
                >
                  {status === 'To Do' && (
                    <span className='px-2 py-1 text-gray-800 rounded-sm bg-gray-300'>To do </span>
                  )}
                  {status === 'In Progress' && (
                  ...
                  {status === 'Done' && (
                    <span className='px-2 py-1 text-green-700 rounded-sm bg-green-200'>done </span>
                  )}
                </a>
              </li>
            ))}
