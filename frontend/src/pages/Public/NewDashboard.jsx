...
export default function NewDashboard(props) {
    const navigation = [
        { name: "Dashboard", href: "/leadership/dashboard", icon: <AiFillHome size={18}/>, current: true },
        { name: 'Projects', href: '/leadership/projects', icon: <BsFolderFill size={18}/>, current: false },
        { name: 'Teams', href: '/leadership/teams', icon: <FaHandshake size={18}/>, current: false },
        { name: 'Community', href: '/leadership/community', icon: <FaUsers size={18}/>, current: true },
      ];
...
//   const { navigation, user, onLogout } = props;
  ...
  <header className="sticky top-0 inset-x-0 flex flex-wrap sm:justify-start sm:flex-nowrap z-[48] w-full bg-white border-b text-sm py-2.5 sm:py-3 lg:pl-0 dark:bg-gray-800 dark:border-gray-700">
    <nav className="flex basis-full items-center w-full mx-auto px-4 sm:px-6 md:px-8" aria-label="Global">
      ...
      <div className="w-full flex items-center justify-end ml-auto sm:justify-between sm:gap-x-3 sm:order-3">
        ...
        <button type="button" className="inline-flex justify-center items-center text-gray-500 hover:text-gray-600" data-hs-overlay="#application-sidebar" aria-controls="application-sidebar" aria-label="Toggle navigation">
        ...
        <span className='ml-2 text-slate-900 font-mono font-semibold text-xl'>Community</span>
      </button>
        ...
        <div className="hidden sm:block">
          ...
          <div className="relative">
            ...
            <input type="text" id="icon" name="icon" className="py-2 px-4 pl-11 block w-full border-gray-200 shadow-sm rounded-md text-sm focus:z-10 focus:border-teal-500 focus:ring-teal-500 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400" placeholder="Search" />
          </div>
        </div>
            ...
            <div className="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-[15rem] bg-white shadow-md rounded-lg p-2 dark:bg-gray-800 dark:border dark:border-gray-700" aria-labelledby="hs-dropdown-with-header">
              ...
              <div className="mt-2 py-2 first:pt-0 last:pb-0">
                {userNavigation.map((item) => (
                  <Link to={item.href} onClick={item.onLogout} className="flex items-center gap-x-3.5 py-2 px-3 rounded-md text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-teal-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300" >
  ...
  <div id="application-sidebar" className="hs-overlay hs-overlay-open:translate-x-0 -translate-x-full transition-all duration-300 transform hidden fixed top-0 left-0 bottom-0 z-[60] w-64 bg-white border-r border-gray-200 pt-7 pb-10 overflow-y-auto scrollbar-y [--body-scroll:true] [--overlay-backdrop:false]" >
    <div className="px-6">
      <a className="flex-none text-xl font-semibold dark:text-white" href="#" aria-label="Brand">Brand</a>
    </div>
    ...
    <nav className="hs-accordion-group p-6 w-full flex flex-col flex-wrap" data-hs-accordion-always-open>
      <ul className="space-y-1.5">
        {navigation.map((item) => (
            <li>
                <Link to={item.href} className={item.current ? "flex items-center gap-x-3.5 py-2 px-2.5 bg-gray-100 text-sm text-slate-700 rounded-md hover:bg-gray-100" : "flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-slate-700 rounded-md hover:bg-gray-100"}>
                    {/* <svg className="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
