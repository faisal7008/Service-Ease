...
export default function Survey() {
  ...
  const { surveys, submissions, isSuccess, isError } = useSelector(
    (state) => state.surveys
  );
  ...
  let filledSurveys = surveys?.filter((survey) => survey?.submissions?.includes(user._id));
  let notFilledSurveys = surveys.filter(survey => !survey?.submissions?.includes(user._id));
  ...
  const onChange = (e, question, choice) => {
    setResponses(
      responses.filter((response) => response.question !== question)
    );
      ...
      <main className="mx-2 lg:mx-28 my-6">
        <Tabs value="Ongoing Surveys">
          <TabsHeader className="bg-slate-200 rounded-b-none">
            <Tab
              className="text-teal-700 font-medium p-1"
              value={"Ongoing Surveys"}
            >
              Ongoing Surveys
            </Tab>
            <Tab
            ...
            <TabPanel value={"Ongoing Surveys"}>
              <div className="hs-accordion-group">
                {notFilledSurveys?.map((survey, key) => (
                  <div
                    className={
                      "hs-accordion hs-accordion-active:bg-white rounded-xl px-5 py-3"
                    }
                    id={`streched-${key + 1}`}
                  >
                    <button
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <Chip
                          className=" text-end rounded bg-teal-500 px-2 py-1"
                          value={`due on ${moment(survey.expirydate).format(
                            "ll"
                          )}`}
                        />
                        <svg
                    ...
                    <div className="">
                      ...
                      >
                        <div
                          id={`collapsed-${key + 1}`}
                          className="hs-accordion-content hidden w-full overflow-auto transition-[height] duration-300"
                        ...
                        >
                          ...
                          <div className="flex justify-end my-4 w-full">
                            <Button
                              type="submit"
                              className=" bg-teal-600 text-white hover:bg-teal-700 rounded px-5 py-3"
                            >
                              submit
                            </Button>
                          </div>
                        ...
                        <table className="min-w-full overflow-auto divide-y divide-gray-200 dark:divide-gray-700">
                          ...
                          <tbody>
                            {submissions
                              .filter(
                                (submission) =>
                                  submission.surveyId === survey._id && submission.employeeId === user._id
                              )
                              .map((submission) =>
                                submission.responses.map((response) => (
                                  <tr className="odd:bg-white even:bg-gray-100 dark:odd:bg-gray-800 dark:even:bg-gray-700">
                                    ...
                                    {choices.map((choice) => (
                                      <td className="px-6 py-1 whitespace-nowrap text-center text-sm text-gray-800 dark:text-gray-200">
                                        <Radio
                                          id={`${response.question + choice}`}
                                          name={response.question}
                                          value={response.answer}
                                          checked={choice === response.answer}
                                        />
                                      </td>
                                    ))}
                                  </tr>
