  ...
  return (
    <main className="messenger flex h-[calc(100vh-70px)]">
      <div className="chatMenu w-3/12 hidden sm:block">
        ...
        <ConversationBox user={user} currentChat={currentChat} setHeading={setHeading} setCurrentChat={setCurrentChat} />
        </div>
      </div>
        <button
          type="button"
          className="py-3 px-4 absolute mt-1 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold text-dark"
      ...
      <div className="chatBox w-full sm:w-9/12">
        <div className="chatBoxWrapper flex flex-col h-full">
          ...
          <div className=" bg-teal-500 text-white w-full shadow-sm shadow-teal-700">
            <h1 className="px-4 m-3 ml-7 sm:m-3 font-mono font-semibold capitalize">{heading}</h1>
          </div> }
          {(heading === "Direct Messages" || heading === "Teams") && <MessageBox currentUser={user} currentChat={currentChat} /> }
