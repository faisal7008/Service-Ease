...
// get user
export const getAllUsers = createAsyncThunk('users/getAll', async (id, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
// get user
export const getUser = createAsyncThunk('users/getOther', async (id, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
// get me
export const getMyProfile = createAsyncThunk('users/getMe', async (_, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
// Get all employees
export const getManagers = createAsyncThunk('users/getManagers', async (_, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
// Get all managers
export const getEmployees = createAsyncThunk('users/getEmployees', async (_, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
// follow user
export const followUser = createAsyncThunk('users/follow', async (userData, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteUser.fulfilled, (state, action) => {
        ...
        state.managers = state.managers.filter((manager) => manager._id !== action.payload.id);
      })
