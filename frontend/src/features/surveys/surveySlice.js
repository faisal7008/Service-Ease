...
// get Surveys
export const getSurveys = createAsyncThunk('surveys/getAll', async (_, thunkAPI) => {
  try {
...
// get Surveys
export const getSubmissions = createAsyncThunk('submissions/getAll', async (_, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
// delete Survey
export const deleteSurvey = createAsyncThunk('surveys/delete', async (SurveyId, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteSurvey.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isSuccess = true;
        state.surveys = state.surveys.filter((Survey) => Survey._id !== action.payload.id);
      })
